...
with tab_compute:
    ...
    student_id = form.text_input("学号", help="请输入学号", key="student_id_2")
    ...
    if compute_button or student_id:
        if student_id == "":
            st.error("学号不能为空")
        try:
            int(student_id)
        ...
        contest_score = contest_result.loc[int(student_id), "单元测验"] * contest_ratio  # 单元测验成绩百分制
        assignment_score = table.loc[int(student_id)].mean() * 10 * (1 - contest_ratio)  # 作业成绩十分制
