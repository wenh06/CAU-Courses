...
.. proof:solution::
    ...
    证明：
        ...
        .. math::
            :label: eq:exam2024-caratheodory-1
        ...
        .. math::
            :label: eq:exam2024-caratheodory-2

            m^* (A \cap E) + m^* (A \cap \mathscr{C} E) \leqslant m^* A.

        由 :eq:`eq:exam2024-caratheodory-1` 和 :eq:`eq:exam2024-caratheodory-2`，知满足卡拉泰奥多里条件成立。
...
.. proof:proof::
    ...
    (1). 对于 :math:`1 \leqslant p < \infty` 的情况，设 :math:`f, g \in L^p`, :math:`a, b \in \mathbb{R}`, 那么
    ...
    第二个不等式是由于函数 :math:`\phi(t) = t^p` 当 :math:`p \geqslant 1` 时是凸函数。所以 :math:`af + bg \in L^p`，这说明 :math:`L^p` 是线性空间。

    对于 :math:`p = \infty` 的情况，设 :math:`f, g \in L^\infty`, :math:`a, b \in \mathbb{R}`, 那么存在 :math:`M_1, M_2 \geqslant 0` 使得
    ...
    (2). 对于 :math:`1 \leqslant p_1 < p_2 < \infty` 的情况，设 :math:`f \in L^{p_2}`, 令 :math:`A = E(\lvert f \rvert \geqslant 1)`, 那么
    ...
    当 :math:`p_2 = \infty` 时，设 :math:`f \in L^\infty`, 那么存在 :math:`M \geqslant 0` 使得 :math:`\lvert f(x) \rvert \leqslant M`, :math:`a.e. x \in E`,
...
.. proof:proof::
    ...
    (1). 记第 :math:`k` 步得到的闭区间为 :math:`F_{k, 1}, F_{k, 2}, \cdots, F_{k, 2^k}`, 去掉的开区间为 :math:`G_{k, 1}, G_{k, 2}, \cdots, G_{k, 2^{k-1}}`, 那么有
    ...
    (2). 首先，很容易观察到 Cantor 函数 :math:`\Phi` 在开集 :math:`G_0` 的每个构成区间 :math:`G_{k, i}` 上都是常值函数，这是因为任取 :math:`x_1, x_2 \in G_{k, i}`,
    有集合的相等关系：
    ...
    这与式 :eq:`exam2024-4-2-1` 矛盾，因此 :math:`\Phi` 不是绝对连续的。
